...
db = {"keys": ["REX-HQUY"], "auth": [], "admins": [OWNER_ID], "delay": 0.2, "anti": []}
user_sessions = {} # {user_id: session_string}
login_step = {}
spam_running = {} # {chat_id: bool} để kiểm soát stop
...
async def handle(e):
    ...
    # 1. LỆNH STOP (FIX CỰC MẠNH)
    if t == '/stop':
        spam_running[cid] = False
        await e.reply("🛑 **SPAM OFF**\nĐã cưỡng chế dừng luồng!"); return

    # 2. LỆNH ẨN & START
    if (is_o or is_a) and t == '/ad': await e.reply(AD_MENU); return
    if t == '/start': await e.reply(MENU_USER if is_v else BANG_GIA); return

    # 3. LOGIN USERBOT (OTP 1.2.3.4.5)
    if t == '/login' and is_v:
    ...
    if t == '/sp':
        ...
        if u in user_sessions:
            uc = TelegramClient(StringSession(user_sessions[u]), API_ID, API_HASH); await uc.connect(); client_to_use = uc
        
        for msg in get_bullets():
            if spam_running.get(cid) == False: break # Check stop liên tục
            try:
        ...
        await e.reply("👑 Đang dọn..."); async for m in bot.iter_messages(cid, from_user='me'): await m.delete()
