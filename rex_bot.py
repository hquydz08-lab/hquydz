...
# --- BẢNG GIÁ CHUẨN THỨ TỰ (2K-10K-20K-70K) ---
...
async def handle(e):
    ...
    # 1. LỆNH ẨN /ad (ƯU TIÊN OWNER/ADMIN)
    if t == '/ad':
    ...
    if is_o:
        if t.startswith('/addadm'):
            ...
            try: aid = int(t.split()[1]); db["admins"].remove(aid); await e.reply(f"➖ Đã xóa Admin: {aid}")
            except: pass
    ...
    if is_a or is_o:
        if t.startswith('/newkey'):
            try: nk = t.split()[1]; db["keys"].append(nk); await e.reply(f"🔑 Key mới: {nk}")
            except: pass
        elif t.startswith('/xoakey'):
            try: nk = t.split()[1]; db["keys"].remove(nk); await e.reply(f"➖ Xóa Key: {nk}")
            except: pass
    ...
    # --- THỰC THI 15 LỆNH (ĐÃ FIX CÚ PHÁP) ---
    if t == '/sp':
    ...
    # FIX LỖI CÚ PHÁP Ở ĐÂY: Thêm async for
    elif t == '/xoaall':
        await e.reply("👑 Đang dọn dẹp..."); 
        async for msg in bot.iter_messages(e.chat_id, from_user='me'): 
            try: await msg.delete()
            except: pass
        await e.reply("✅ Đã dọn xong!")
    ...
    elif t == '/login': await e.reply("👑 Gửi SĐT để Login...")
